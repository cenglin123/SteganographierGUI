...
上游 downkyi 1.6.x 存在"扫码后登录信息未保存、手机上确认了登录但程序仍返回未登录"的问题，公开讨论见
[leiurayer/downkyi#1874](https://github.com/leiurayer/downkyi/issues/1874)（该 issue 的回复里附有一份社区补丁）。
**包内 `DownKyi.Core.dll` 的改动对应的就是这一类问题。**

**但这不等于"包内这份就是 #1874 的附件"**——不写清楚的话，下次有人拿它去比对会得出错误结论。三份文件实测互不相同：

| 来源 | 大小 | SHA256 |
|---|---|---|
| 上游 `v1.6.1` 归档内的原件 | 542,720 | `6fe0e72c6c664ad8a5c6cd03f4e6a0fa4abf1577559cd4dc0d82524dfb503180` |
| **包内作者版（git override）** | 597,504 | `3681e5fe31c97491b117954003bdf36871d527742fe7638d9b0c45eaabf12f91` |
| #1874 附件 `fix.zip` 内 | 592,384 | `400ac85638f77a0e6d921ee02aff6644b48958490c54c4b8c6d1e345ed4602a1` |

三者的 `FileVersion` 都是 `2.2.1.0`，**版本号区分不出来，只能比字节**。该附件另附一个
`DownKyi.LoginFix.dll`（7,168 字节），而作者的包内没有这个文件（36 个文件与作者认可构建逐字节一致），
这也说明二者不是同一份改动。结论：**不要把 #1874 的附件当作我们这份的来源或替代品。**

另有两点来自该 thread，供参考：

- 那份社区补丁的作者自述**取消了登录文件的硬件绑定**，并提醒不要外传生成的 `Config/Login`。
  若要把它打在包内这份之上，需先明白这一点。（包内这份是否做了同样的事，本文档不作断言。）
- thread 中有人报告 GitHub 上的 1.6.1 出现 `DownKyi.exe` 与 `DownKyi.Core.dll` 版本不匹配并崩溃，
  那是他自己打了 [#1845](https://github.com/leiurayer/downkyi/issues/1845) 补丁的构建，不是官方 asset：
  上游 `v1.6.1` Release **只有 `DownKyi-1.6.1.zip` 一个附件**，大小与本文档固定的 32,204,639 字节一致。
  我们的流水线对 36 个文件逐个核对 MANIFEST，这类不一致只会让构建失败，不会出厂。

## 落地方式（为什么只把 1 个文件放进 git）

| 部分 | 存放方式 | 理由 |
|---|---|---|
| 作者的 `DownKyi.Core.dll` | **git 跟踪**：`vendor/downkyi-1.6.1/overrides/DownKyi.Core.dll`（583 KB） | 它**无法从任何公开来源复现**（已核对上游 `v1.6.1` 归档内的原件与 #1874 的附件，两份都不是它），只有 git 能作为单一事实来源长期保存 |
